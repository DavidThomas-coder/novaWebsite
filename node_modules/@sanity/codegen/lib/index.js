...
class TypeGenerator {
  ...
  generateSchemaTypes() {
    ...
    return this.schema.forEach((schema) => {
      const typeLiteral = this.getTypeNodeType(schema), schemaName = this.getTypeName(schema.name, schema);
...
`);
  ...
  generateTypeNodeTypes(identifierName, typeNode) {
    const type = this.getTypeNodeType(typeNode), typeAlias = t__namespace.tsTypeAliasDeclaration(
      t__namespace.identifier(this.getTypeName(identifierName, typeNode)),
      null,
      type
    );
  ...
  getTypeName(name, typeNode) {
    ...
    return this.generatedTypeName.add(generatedName), typeNode && this.typeNameMap.set(typeNode, generatedName), generatedName;
  }
          ...
          return t__namespace.tsIntersectionType([
            ...
            t__namespace.tsTypeReference(
              t__namespace.identifier(
                this.typeNameMap.get(referencedTypeNode) || uppercaseFirstLetter(sanitizeIdentifier(rest.name))
              )
            )
          ]);
        }
