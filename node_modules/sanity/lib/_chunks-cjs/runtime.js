...
function getAliases({ monorepo, sanityPkgPath }) {
  ...
  if (sanityPkgPath) {
    ...
    return browserCompatibleSanityPackageSpecifiers.reduce(
      (acc, next) => {
        var _a;
        const dest = (_a = resolve__default.default.exports(pkg, next, { browser: !0, conditions })) == null ? void 0 : _a[0];
...
function createExternalFromImportMap({ imports = {} } = {}) {
  return Object.keys(imports).map(
    (specifier) => specifier.endsWith("/") ? new RegExp(`^${escapeRegExp__default.default(specifier)}.+`) : specifier
  );
}
...
function renderDocument(options) {
  return new Promise((resolve2, reject) => {
    if (!useThreads) {
      resolve2(getDocumentHtml(options.studioRootPath, options.props, options.importMap));
      return;
    }
...
function renderDocumentFromWorkerData() {
  ...
  const { monorepo, studioRootPath, props, importMap } = node_worker_threads.workerData || {};
  ...
  const { unregister: unregisterJs } = require("esbuild-register/dist/node").register({
    ...
    jsx: "automatic",
    loader: "jsx"
  }), html = getDocumentHtml(studioRootPath, props, importMap);
  node_worker_threads.parentPort.postMessage({ type: "result", html }), unregister(), unregisterJs();
}
function getDocumentHtml(studioRootPath, props, importMap) {
  var _a;
  ...
  return debug$2("Rendering document component using React"), `<!DOCTYPE html>${addTimestampedImportMapScriptToHtml(
    server.renderToStaticMarkup(React.createElement(Document, { ...defaultProps, ...props, css })),
    importMap
  )}`;
}
function addTimestampedImportMapScriptToHtml(html, importMap) {
  ...
  return headEl || (htmlEl.insertAdjacentHTML("afterbegin", "<head></head>"), headEl = root.querySelector("head")), headEl.insertAdjacentHTML(
    "beforeend",
    `<script type="application/json" id="__imports">${JSON.stringify(importMap)}<\/script>`
  ), headEl.insertAdjacentHTML("beforeend", TIMESTAMPED_IMPORTMAP_INJECTOR_SCRIPT), root.outerHTML;
}
