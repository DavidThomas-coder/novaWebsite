...
function handleFuncCallNode(node, scope) {
  switch (`${node.namespace}.${node.name}`) {
    ...
    case "global.dateTime": {
      const arg = walk({ node: node.args[0], scope });
      return mapConcrete(arg, scope, (arg2) => arg2.type === "string" ? nullUnion({ type: "string" }) : { type: "null" });
    }
    case "global.length": {
      const arg = walk({ node: node.args[0], scope });
      return mapConcrete(arg, scope, (arg2) => arg2.type === "array" ? { type: "number" } : arg2.type === "string" ? { type: "number" } : { type: "null" });
    }
